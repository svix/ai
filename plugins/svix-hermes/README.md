    ...
    extra:
      ...
      routes:
        github_events:
          ...
          # Subscribed server-side as the endpoint's filter_types, so Svix
          # only delivers these events (omit for all event types).
...
| `events` | Optional list of `eventType`s. Pushed to Svix as the endpoint's `filter_types` at `subscribe()` (so the server only delivers these) and re-checked client-side. Omit for all event types. |
